...
say "4. deploy (with --features test-hooks, per icp.yaml)"
...
say "6. seal a secret"
icp identity export "$(icp identity default)" > "$PEM"
( cd seed && [ -d node_modules ] || npm install --silent >/dev/null 2>&1 )
...
say "8. it survives an upgrade with no re-seeding"
icp deploy -e "$ENV" >/dev/null
