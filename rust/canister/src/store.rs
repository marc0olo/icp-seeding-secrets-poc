//! Stable state: the key name, and the decrypted secrets.
//!
//! Secrets live here so that an upgrade never requires re-seeding. They are held
//! decrypted — see `SealedRecord` for why storing the ciphertext instead would
//! protect nothing.
...
/// `sealed_secrets_core::CONTEXT` and `KEY_LABEL`. The key name cannot be, since
/// mainnet and a local network both have a `key_1` backed by different master
/// keys, and only the deployer knows which network this is.
///
/// Written through `StableCell::init`, which *keeps* an existing value: editing
/// the constant in source and upgrading is a silent no-op, deliberately, so that
/// an upgrade cannot change the derivation under stored secrets.
