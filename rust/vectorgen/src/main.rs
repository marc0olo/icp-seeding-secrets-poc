...
///
/// Generated through `sealed_secrets_core::derive_public_key`, the offline
/// derivation a client runs — not by calling `ic-vetkeys` directly — so these
/// vectors also pin the PoC's own context encoding.
///
...
fn emit_derivation_vectors() {
    use sealed_secrets_core::{derive_public_key, key_id, MasterKeySource, CONTEXT};
