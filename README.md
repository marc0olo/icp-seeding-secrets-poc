...
field tower, both curve groups, the optimal ate pairing and IBE decryption, at
1.79 billion instructions per decryption against the 40 billion an update call
gets. It decrypts a ciphertext generated by the Rust reference, which is the only
test that really counts.

It is a demonstration, not a recommendation — read
[motoko/README.md](./motoko/README.md) before considering it for anything, and
note that `hash_to_curve` is still missing, so a Motoko canister could decrypt but
could not yet verify that the subnet returned a genuine vetKey.
