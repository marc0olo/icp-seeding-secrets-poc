...
The Rust canister here is the reference implementation, but the pattern should
not be Rust-only — and the obstacle was that Motoko has no BLS12-381, so a Motoko
canister could receive a sealed secret and never open it.

`motoko/` now holds an **experimental, unaudited** port that closes that gap: the
field tower, both curve groups, the optimal ate pairing, RFC 9380 hash-to-curve,
IBE decryption and vetKey verification — every cryptographic operation this PoC's
Rust canister performs, with no functional gap left between them. It costs about
5.4 billion instructions cold (verify plus decrypt) against the 40 billion an
update call gets, and it is paid once.

It is a library, not a canister. The one non-cryptographic piece, the
`vetkd_derive_key` call itself, is an ordinary management-canister call Motoko
has always been able to make via `ManagementCanister.mo` — it was never the
obstacle.

Two tests carry the weight: it decrypts a ciphertext generated by the Rust
reference, and it verifies a real `vetkd_derive_key` reply taken from
`ic-vetkeys`' own test suite.
...
crates/vectorgen/      emits motoko/test/vectors.json from the Rust reference.
