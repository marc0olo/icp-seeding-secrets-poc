...
**It is not yet a drop-in replacement for the Rust canister here.** It has no
counterpart to the offline derived-public-key computation
(`derive_canister_key` / `derive_sub_key`), which is how this canister checks the
subnet's reply against a master key compiled into its own Wasm rather than asking
the subnet to vouch for itself. That is a small addition — see
[motoko/README.md](./motoko/README.md#coverage) — but it is not written.
