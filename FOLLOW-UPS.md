...
and `decrypt_and_verify`. Every layer is tested against vectors generated from
`ic_bls12_381` itself, the hash layer against Python's `hashlib` — a third
implementation unrelated to either — and the verification against a real
`vetkd_derive_key` reply lifted from `ic-vetkeys`' own tests, where it is
annotated as having been generated by the replica's threshold implementation.
...
billion an update call gets, and paid once because both results are cached. On
the decryption alone, the like-for-like comparison, that is about 10.8× the Rust
implementation measured the same way, and 85% of the gap is one thing: this port
reduces with `%` where the reference uses Montgomery form, so it divides where
the reference multiplies. Fixing that, not switching to limb representation, is
