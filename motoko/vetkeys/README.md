...
Upstream those come from the `hkdf` and `sha3` crates rather than from
...
network with no configuration naming which. The sealed-secrets canister in
[`../canister`](../canister) takes that route.

A **client** cannot. It has no `vetkd_public_key` to call, and trusting a key a
canister reported would defeat the point. It must derive offline, from a master
public key compiled into its own binary, which is what `PublicKey` is for — and
what `mo:ic-vetkeys` has no equivalent of, so a Motoko client cannot seal
anything today without it.
...
`protocol_flow_with_fixed_rng_has_expected_outputs` (`tests/utils.rs:240`), where they are
annotated "generated by internal library" — which we read as the threshold
implementation running in the replica. `test/VetKey.test.mo` recovers exactly that vetKey from them.
