...
/// This port stores a plain `Nat`. Reducing with `%` is a division by a 381-bit
/// modulus; `Fp.mul` uses Barrett reduction instead.
///
/// This separates the halves so the difference is measured rather than
/// assumed.
...
module {
  public func init() : Bench.Bench {
    ...
    bench.rows(["mul_then_reduce", "mul_barrett", "mul_only", "reduce_only", "add"]);
    bench.cols(["100"]);
      ...
      func(row, _col) {
        ...
        while (i < 100) {
          switch (row) {
            ...
            case ("mul_barrett") { ignore Fp.mul(x, y) };
