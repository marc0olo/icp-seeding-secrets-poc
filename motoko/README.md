...
The port now covers the whole canister-side path the Rust PoC uses: unwrap the
subnet's reply, **verify** it, then decrypt the sealed secret. There is no
remaining functional gap between the two.

Getting there needed `hash_to_curve`, which is about 3,200 of the reference's
lines and the single largest piece of the port. It is not used by
`IbeCiphertext::decrypt` — that needs only a pairing and a `G2` scalar
multiplication — which is why an earlier revision of this port stopped short of
it and could decrypt without being able to verify. That is a real difference in a
security property: a canister without it takes the subnet's word for the key it
was handed.

The verification vectors are worth a note. They are not generated by this
repository. They are lifted from `ic-vetkeys`' own
...
in the replica. So the port is checked against a reply a subnet actually
produced, and the negative cases (wrong transport key, wrong identity,
substituted derived public key, tampered `c1`/`c3`) confirm the check rejects
what it must, which a version that skipped verification entirely would pass just
as happily on the positive case.
